...
// JPortalDump
  ...
  {
    JPortalDumper::InterpreterInfo inter(sizeof(JPortalDumper::InterpreterInfo));
    int inter_pointer = 0;
    inter.codelets_address[inter_pointer++] = (uint64_t)CodeCache::low_bound();
    inter.codelets_address[inter_pointer++] = (uint64_t)CodeCache::high_bound();
#ifndef PRODUCT
    inter.TraceBytecodes = TraceBytecodes;
#elif
    inter.TraceBytecodes = false;
#endif
    _code->stub_addresses(inter.codelets_address, inter_pointer);
    int dispatch_cnt = 0;
    int dispatch_state;
    for (dispatch_cnt = 0; dispatch_cnt < (int)Bytecodes::number_of_codes; dispatch_cnt++)
      for (dispatch_state = 0; dispatch_state < number_of_states; dispatch_state++)
        inter.codelets_address[inter_pointer++] = (uint64_t)_normal_table.entry(dispatch_cnt).entry((TosState)dispatch_state);
    inter.codelets_address[inter_pointer++] = (uint64_t)_slow_signature_handler;
#ifndef PRODUCT
    if (TraceBytecodes)
      for (dispatch_state = 0; dispatch_state < number_of_states; dispatch_state++)
        inter.codelets_address[inter_pointer++] = (uint64_t)_trace_code.entry((TosState)dispatch_state);
#endif
    for (dispatch_cnt = 0; dispatch_cnt < number_of_return_entries; dispatch_cnt++)
      for (dispatch_state = 0; dispatch_state < number_of_states; dispatch_state++)
        inter.codelets_address[inter_pointer++] = (uint64_t)_return_entry[dispatch_cnt].entry((TosState)dispatch_state);
    
    for (dispatch_cnt = 0; dispatch_cnt < number_of_return_addrs; dispatch_cnt++) {
      inter.codelets_address[inter_pointer++] = (uint64_t)_invoke_return_entry[dispatch_cnt];
      inter.codelets_address[inter_pointer++] = (uint64_t)_invokeinterface_return_entry[dispatch_cnt];
      inter.codelets_address[inter_pointer++] = (uint64_t)_invokedynamic_return_entry[dispatch_cnt];
    }
    for (dispatch_state = 0; dispatch_state < number_of_states; dispatch_state++)
      inter.codelets_address[inter_pointer++] = (uint64_t)_earlyret_entry.entry((TosState)dispatch_state);
    for (dispatch_cnt = 0; dispatch_cnt < number_of_result_handlers; dispatch_cnt++)
      inter.codelets_address[inter_pointer++] = (uint64_t)_native_abi_to_tosca[dispatch_cnt];
    for (dispatch_state = 0; dispatch_state < number_of_states; dispatch_state++)
      inter.codelets_address[inter_pointer++] = (uint64_t)_safept_entry.entry((TosState)dispatch_state);
    inter.codelets_address[inter_pointer++] = (uint64_t)Interpreter::_rethrow_exception_entry; 
    inter.codelets_address[inter_pointer++] = (uint64_t)Interpreter::_throw_exception_entry;
    inter.codelets_address[inter_pointer++] = (uint64_t)Interpreter::_remove_activation_preserving_args_entry;
    inter.codelets_address[inter_pointer++] = (uint64_t)Interpreter::_remove_activation_entry;
    inter.codelets_address[inter_pointer++] = (uint64_t)Interpreter::_throw_ArrayIndexOutOfBoundsException_entry;
    inter.codelets_address[inter_pointer++] = (uint64_t)Interpreter::_throw_ArrayStoreException_entry;
    inter.codelets_address[inter_pointer++] = (uint64_t)Interpreter::_throw_ArithmeticException_entry;
    inter.codelets_address[inter_pointer++] = (uint64_t)Interpreter::_throw_ClassCastException_entry;
    inter.codelets_address[inter_pointer++] = (uint64_t)Interpreter::_throw_NullPointerException_entry;
    inter.codelets_address[inter_pointer++] = (uint64_t)Interpreter::_throw_StackOverflowError_entry;
    for (dispatch_cnt = 0; dispatch_cnt < number_of_method_entries; dispatch_cnt++)
      inter.codelets_address[inter_pointer++] = (uint64_t)_entry_table[dispatch_cnt];
    for (dispatch_cnt = 0; dispatch_cnt < number_of_deopt_entries; dispatch_cnt++)
      for (dispatch_state = 0; dispatch_state < number_of_states; dispatch_state++)
        inter.codelets_address[inter_pointer++] = (uint64_t)_deopt_entry[dispatch_cnt].entry((TosState)dispatch_state);
  JPortalDumper::jportal_interpreter_codelets(inter);
  }
#endif
#endif
  if (PrintInterpreter) {
