...
#if defined(linux)
#if defined(__x86_64__)
void jportalDumper_init() {
  JPortalDumper::jportal_initialize();
}

void jportalDumper_exit() {
  JPortalDumper::jportal_exit();
}
#endif
#endif
...
int JPortalDumper::shm_id = -1;
address JPortalDumper::shm_addr = NULL;
size_t JPortalDumper::shm_volume = 1024*1024*4;
address JPortalDumper::data_begin = NULL;
address JPortalDumper::data_end = NULL;
size_t JPortalDumper::data_volume = 0;
pthread_mutex_t JPortalDumper::dumper_lock;
int JPortalDumper::dump_pipe[2];
GrowableArray<Method *> *JPortalDumper::method_array;

inline jlong JPortalDumper::get_java_tid(JavaThread* thread) {
  oop obj = ((JavaThread*)thread)->threadObj();
  return (obj == NULL) ? 0 : java_lang_Thread::thread_id(obj);
}

void JPortalDumper::jportal_interpreter_codelets(InterpreterInfo inter) {
  if (!JPortalDump)
    return;

  uint64_t size = sizeof(InterpreterInfo);
  if (data_volume <= size) {
    fprintf(stderr, "JPortal Dump: ignore compiled code for size too big.\n");
    return;
  }
  ...
  while (1) {
    ...
    if (data_size > size)
      ...
      if (write(dump_pipe[1], &bf, 1) <= 0) {
        jportal_exit();
        fprintf(stderr, "JPortal Dump: compiled code write pipe error.\n");
        return;
      }
    }
  }

  address addr = data_begin + data_head;
  addr = copy_data(addr, (address)&inter, sizeof(inter));

  header->data_head = (addr - data_begin);
  if (write(dump_pipe[1], &bf, 1) <= 0) {
    jportal_exit();
    fprintf(stderr, "JPortal Dump: compiled code write pipe error.\n");
    return;
  }
  if (pthread_mutex_unlock(&dumper_lock) < 0) {
    jportal_exit();
    fprintf(stderr, "JPortal Dump: unlock error.\n");
  }
}

void JPortalDumper::jportal_method_entry(JavaThread *thread, Method *moop) {
  if (!JPortalDump || !JPortalMethod || !thread || !moop)
    return;

  bool method_filter = false;
  bool class_filter = false;
  if (JPortalMethodFilters && JPortalMethodFilters[0] != '\0') {
    const char *position = JPortalMethodFilters;
    const char *filters = position;
    while (position) {
      if (*position == ',' || *position == '\0') {
        if (memcmp(filters, moop->name()->bytes(), position-filters) == 0) {
          method_filter = true;
          break;
        }
        filters = position+1;
  ...
  if (JPortalClassFilters && JPortalClassFilters[0] != '\0') {
    const char *position = JPortalClassFilters;
    const char *filters = position;
    while (position) {
      if (*position == ',' || *position == '\0') {
        if (memcmp(filters, moop->klass_name()->bytes(), position-filters) == 0) {
          class_filter = true;
          break;
        }
        filters = position+1;
  ...
  if (idx == -1) {
    ...
    uint64_t size = sizeof(MethodEntryInitial) +
                      klass_name_length + name_length + sig_length;
    MethodEntryInitial me(idx, get_java_tid(thread), 
                    klass_name_length, name_length, sig_length, size);

    if (data_volume <= size) {
      fprintf(stderr, "JPortal Dump: ignore entry for size too big.\n");
      if (pthread_mutex_unlock(&dumper_lock) < 0) {
        jportal_exit();
        fprintf(stderr, "JPortal Dump: unlock error.\n");
      }
    ...
    while (1) {
      ...
      if (data_head < data_tail)
        data_size = data_tail - data_head;
      else
        data_size = data_volume - (data_head - data_tail);
      if (data_size > size)
        break;
      else {
        if (write(dump_pipe[1], &bf, 1) <= 0) {
          jportal_exit();
          fprintf(stderr, "JPortal Dump: entry write pipe error.\n");
          return;
        }
      }
    }
    ...
    address addr = (data_begin + data_head);
    addr = copy_data(addr, (address)&me, sizeof(me));
    addr = copy_data(addr, (address)klass_name, klass_name_length);
    addr = copy_data(addr, (address)method_name, name_length);
    addr = copy_data(addr, (address)method_signature, sig_length);
  ...
  uint64_t size = sizeof(MethodEntryInfo);
...
void JPortalDumper::jportal_method_exit(JavaThread *thread, Method *moop) {
  if (!JPortalDump || !JPortalMethod || !thread || !moop)
    return;
  ...
  uint64_t size = sizeof(MethodExitInfo);
...
void JPortalDumper::jportal_compiled_method_load(Method *moop,
                    address code_begin, size_t code_size,
                    address scopes_pc_begin, size_t scopes_pc_size,
                    address scopes_data_begin, size_t scopes_data_size,
                    Metadata **metadata_begin, int metadata_cnt,
                    address entry_point, address verified_entry_point,
                    address osr_entry_point) {
  if (!JPortalDump || !moop)
    return;

  uint64_t size = sizeof(CompiledMethodLoadInfo) +
  ...
  CompiledMethodLoadInfo cm((uint64_t)code_begin, code_size, 
                    scopes_pc_size, scopes_data_size,
  ...
  address addr = data_begin + data_head;
  addr = copy_data(addr, (address)&cm, sizeof(cm));

  if (inline_method_cnt == 0) {
    InlineMethodInfo mi(moop->klass_name()->utf8_length(),
                  moop->name()->utf8_length(),
                  moop->signature()->utf8_length(),
                  1);
    addr = copy_data(addr, (address)&mi, sizeof(mi));
    addr = copy_data(addr, (address)moop->klass_name()->bytes(),
                      moop->klass_name()->utf8_length());
    addr = copy_data(addr, (address)moop->name()->bytes(),
                      moop->name()->utf8_length());
    addr = copy_data(addr, (address)moop->signature()->bytes(),
                      moop->signature()->utf8_length());
    inline_method_cnt = 1;
  } else {
    for (int index = 0; index < metadata_cnt; index++) {
      ...
      InlineMethodInfo mi(m->klass_name()->utf8_length(),
                    m->name()->utf8_length(),
                    m->signature()->utf8_length(),
                    index+1);
      addr = copy_data(addr, (address)&mi, sizeof(mi));
      addr = copy_data(addr, (address)m->klass_name()->bytes(),
                      m->klass_name()->utf8_length());
      addr = copy_data(addr, (address)m->name()->bytes(),
                      m->name()->utf8_length());
      addr = copy_data(addr, (address)m->signature()->bytes(),
                      m->signature()->utf8_length());
    }
  }
  addr = copy_data(addr, code_begin, code_size);
  addr = copy_data(addr, scopes_pc_begin, scopes_pc_size);
  addr = copy_data(addr, scopes_data_begin, scopes_data_size);
  header->data_head = (addr - data_begin);
  if (write(dump_pipe[1], &bf, 1) <= 0) {
    jportal_exit();
    fprintf(stderr, "JPortal Dump: compiled code write pipe error.\n");
...
void JPortalDumper::jportal_compiled_method_unload(Method *moop,
                    address code_begin) {
  if (!JPortalDump || !moop) {
    return;
  }

  uint64_t size = sizeof(CompiledMethodUnloadInfo);
  CompiledMethodUnloadInfo cmu((uint64_t)code_begin, size);
  ...
  address addr = data_begin + data_head;
  addr = copy_data(addr, (address)&cmu, size);
  if (write(dump_pipe[1], &bf, 1) <= 0) {
    jportal_exit();
    fprintf(stderr, "JPortal Dump: compiled code write pipe error.\n");
    return;
...
void JPortalDumper::jportal_thread_start(JavaThread *thread) {
  if (!JPortalDump || !data_volume)
    return;

  uint64_t size = sizeof(ThreadStartInfo);
  ThreadStartInfo ts(get_java_tid(thread), syscall(SYS_gettid), size);

  if (data_volume <= size) {
    fprintf(stderr, "JPortal Dump: ignore thread start for size too big.\n");
    return;
  }
  ...
  while (1) {
    ...
    if (data_size > size)
      ...
      if (write(dump_pipe[1], &bf, 1) <= 0) {
        jportal_exit();
        fprintf(stderr, "JPortal Dump: thread start write pipe error.\n");
        return;
      }
    }
  }
  ...
  address addr = data_begin + data_head;
  addr = copy_data(addr, (address)&ts, sizeof(ts));
...
void JPortalDumper::jportal_dynamic_code_generated(const char *name,
                    address code_begin, size_t code_size) {
  if (!JPortalDump || !data_volume)
    return;

  int name_length = strlen(name) + 1;
  uint64_t size = sizeof(DynamicCodeGenerated) + name_length + code_size;
  DynamicCodeGenerated dcg(name_length, (uint64_t)code_begin,
                            code_size, size);

  if (data_volume <= size) {
    fprintf(stderr, "JPortal Dump: ignore dynamic code for size too big %ld.\n", size);
    return;
  }
  ...
  while (1) {
    ...
    if (data_size > size)
      ...
      if (write(dump_pipe[1], &bf, 1) <= 0) {
        jportal_exit();
        fprintf(stderr, "JPortal Dump: dynamic code write pipe error.\n");
        return;
      }
    }
  }
  ...
  address addr = data_begin + data_head;
  addr = copy_data(addr, (address)&dcg, sizeof(dcg));
  addr = copy_data(addr, (address)name, name_length);
  addr = copy_data(addr, code_begin, code_size);
...
void JPortalDumper::jportal_inline_cache_add(address src, address dest) {
  if (!JPortalDump || !data_volume)
    return;

  uint64_t size = sizeof(InlineCacheAdd);
  InlineCacheAdd ic((uint64_t)src, (uint64_t)dest, size);
  ...
  address addr = data_begin + data_head;
  addr = copy_data(addr, (address)&ic, sizeof(ic));
...
void JPortalDumper::jportal_inline_cache_clear(address src) {
  if (!JPortalDump || !data_volume)
    return;

  uint64_t size = sizeof(InlineCacheClear);
  InlineCacheClear ic((uint64_t)src, size);
...
void JPortalDumper::jportal_initialize() {
  if (!JPortalDump)
    return;

  shm_id = shmget(IPC_PRIVATE, shm_volume, IPC_CREAT|0600) ;
  if (shm_id < 0) {
    JPortalDump = false;
    fprintf(stderr, "JPortal Dump: shared memory get error.\n");
    return;
  }
  ...
  if (pipe(dump_pipe) < 0) {
    ...
    JPortalDump = false;
    fprintf(stderr, "JPortal Dump: dump pipe open error.\n");
    return;
  }
  ...
  if (dumper_pid == 0) {
    ...
    char shmid_arg[16];
    char dump_fd[16];
    char shmvolume_arg[32];
    if (sprintf(shmid_arg, "%d", shm_id) < 0 ||
          sprintf(dump_fd, "%d", dump_pipe[0]) < 0 ||
          sprintf(shmvolume_arg, "%lu", shm_volume) < 0) {
      fprintf(stderr, "JPortal Dump: internal error.\n");
      close(dump_pipe[0]);
      exit(-1);
    }
    ...
    close(dump_pipe[0]);
    exit(-1);
  } else if (dumper_pid < 0) {
    close(dump_pipe[0]);
    close(dump_pipe[1]);
    shmctl(shm_id, IPC_RMID, NULL);
    JPortalDump = false;
    fprintf(stderr, "JPortal Dump: fork dump process error.\n");
    return;
  }

  method_array = new(ResourceObj::C_HEAP, mtInternal) GrowableArray<Method*> (10, true);

  shm_addr = (address)shmat(shm_id, NULL, 0);
  data_begin = shm_addr + sizeof(ShmHeader);
  data_end = shm_addr + shm_volume;
  data_volume = shm_volume - sizeof(ShmHeader);

  ShmHeader *header = (ShmHeader *)shm_addr;
  header->data_head = 0;
  header->data_tail = data_volume;

  close(dump_pipe[0]);
  if (pthread_mutex_init(&dumper_lock, NULL) < 0) {
    close(dump_pipe[1]);
    shmdt(shm_addr);
    shmctl(shm_id, IPC_RMID, NULL);
    JPortalDump = false;
...
void JPortalDumper::jportal_exit() {
  if (!JPortalDump)
    return;

  delete method_array;
  close(dump_pipe[1]);
  pthread_mutex_destroy(&dumper_lock);
  shmdt(shm_addr);
  shmctl(shm_id, IPC_RMID, NULL);
  JPortalDump = false;
}

void JPortalTracer::jportal_enable() {
  if (JPortalTrace) {
    int trace_pipe[2];
    if (pipe(trace_pipe) < 0)
      fprintf(stderr, "JPortalTrace: open trace pipe error.\n");
    else {
      char java_pid[20];
      char write_pipe[20];
      char _low_bound[20], _high_bound[20];
      if (sprintf(java_pid, "%ld", syscall(SYS_gettid)) < 0 ||
            sprintf(write_pipe, "%d", trace_pipe[1]) < 0 ||
            sprintf(_low_bound, "%p", CodeCache::low_bound()) < 0 ||
            sprintf(_high_bound, "%p", CodeCache::high_bound()) < 0)
        fprintf(stderr, "JPortal: internal error.\n");
      else {
        pid_t pid = fork();
        if (pid == 0) {
          ...
          close(trace_pipe[1]);
          fprintf(stderr, "JPortal: trace process execution error.\n");
          exit(-1);
        } else if (pid < 0) {
          close(trace_pipe[0]);
          close(trace_pipe[1]);
          fprintf(stderr, "JPortal: fork trace process error.\n");
        } else {
          char bf;
          close(trace_pipe[1]);
          if (read(trace_pipe[0], &bf, 1) != 1)
            fprintf(stderr, "JPotal: trace pipe read error.\n");
          close(trace_pipe[0]);
          JPortalTrace = false;
        }
      }
