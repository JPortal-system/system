...
int jit_section_attach(struct jit_section *section, struct jit_image *image) {
    ...
    errcode = jit_section_lock_attach(section);
    if (errcode < 0)
        return errcode;
    ...
    if (!acount) {
        if (section->image || !ucount)
            goto out_unlock;

        section->image = image;
        section->acount = 1;

        return jit_section_unlock_attach(section);
    }

    acount += 1;
    if (!acount) {
        (void)jit_section_unlock_attach(section);
        return -pte_overflow;
    }
...
int jit_section_detach(struct jit_section *section, struct jit_image *image) {
    uint16_t acount, ucount;
    int errcode;
